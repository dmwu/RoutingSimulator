...
RandomQueue::RandomQueue(linkspeed_bps bitrate, mem_b maxsize, EventList &eventlist,uint32_t gid, QueueLogger *logger, mem_b drop)
        : Queue(bitrate, maxsize, eventlist, gid, logger),
          _drop(drop),
          _buffer_drops(0) {
    _drop_th = _maxsize - _drop;
    _plr = 0.0;
