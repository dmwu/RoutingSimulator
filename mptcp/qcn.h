...
public:
  ...
  inline static QcnAck* newpkt(QcnPacket &datapkt, fb_t fb) {
	...
	p->set(*datapkt._flow, *routeback, QcnAck::ACKSIZE, datapkt._seqno);
  ...
  virtual ~QcnAck(){};
  void free() {_packetdb.freePacket(this);}
  const static int ACKSIZE;
  void sendOn() { 
	assert(_nexthop<=_route->size());
	PacketSink* nextsink;
	if (_nexthop==_route->size()) nextsink=_reactor; else nextsink=(*_route)[_nexthop];
	_nexthop++;
