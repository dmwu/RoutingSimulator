...
SubflowControl::SubflowControl(simtime_picosec scanPeriod, Logfile* lg, SinkLoggerSampling* sl,EventList& eventlist, TcpRtxTimerScanner* rtx,  vector<route_t*>*** n, std::ofstream* p, int ms) : EventSource(eventlist,"SubflowControl"), _scanPeriod(scanPeriod)
{
...
void SubflowControl::run(){
  ...
  for (it = flow_counters.begin();it!=flow_counters.end();it++){
    ...
    unsigned int counts = f->byte_counter!=0;
	...
	choice = choice * K/2 + rand()%2;
      ...
      route_t* routeout = new route_t(*(net_paths[f->src][f->dest]->at(choice)));
