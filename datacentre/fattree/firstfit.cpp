...
FirstFit::FirstFit(simtime_picosec scanPeriod, EventList &eventlist, vector<route_t *> ***n) : EventSource(eventlist,
                                                                                                           "FirstFit"),
...
void FirstFit::add_queue(RandomQueue *queue) {
    assert(queue != NULL);
    path_allocations[queue] = 0; //record how many flows go through this queue
}
    ...
    for (it = flow_counters.begin(); it != flow_counters.end(); it++) {
        ...
        if (f->allocated && delta < threshold) {
            ...
            for (unsigned int i = 1; i < f->route->size() - 1; i += 2) {
                path_allocations[(RandomQueue *) f->route->at(i)] -= 1;
            }
        }
    }
    ...
    for (it = flow_counters.begin(); it != flow_counters.end(); it++) {
        ...
        if (!f->allocated && delta > threshold) {
            ...
            for (unsigned int p = 0; p < net_paths[f->src][f->dest]->size(); p++) {
                route_t *crt_route = net_paths[f->src][f->dest]->at(p);
                crt_cost = 0;

                for (unsigned int i = 1; i < crt_route->size() - 1; i += 2)
                    if (path_allocations[(RandomQueue *) crt_route->at(i)] > crt_cost)
                        crt_cost = path_allocations[(RandomQueue *) crt_route->at(i)];
            ...
            route_t *new_route = new route_t(*(net_paths[f->src][f->dest]->at(best_route)));
            ...
            for (unsigned int i = 1; i < tcp->_route->size() - 1; i += 2) {
                path_allocations[(RandomQueue *) new_route->at(i)] += 1;
            }
        }
    }
}
