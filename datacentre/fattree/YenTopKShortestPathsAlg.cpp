...
void YenTopKShortestPathsAlg::_init()
{
	...
	if (m_pSourceVertex != NULL && m_pTargetVertex != NULL)
	{
		...
		else { // Ankit: Silent failures are bad! Is there no shortest path?
			cout << "Yen Algo failed to find shortest paths between " << m_pSourceVertex->getID() << " and " << m_pTargetVertex->getID() << endl;
		}
	}
	else { // Ankit: Silent failures are bad!
		cout << "Yen Algo did not find the src OR dest vertex in its graph!" << endl;
	}
}
...
BasePath* YenTopKShortestPathsAlg::next()
{
	...
	unsigned int sub_path_length = sub_path_of_derivation_pt.size();
