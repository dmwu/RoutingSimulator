...
ShortFlows::ShortFlows(double lambda, EventList& eventlist, vector<route_t*>*** n,ConnectionMatrix* conns,Logfile* logfile,TcpRtxTimerScanner * rtx) : EventSource(eventlist,"ShortFlows")
{
...
ShortFlow* ShortFlows::createConnection(int src, int dst, simtime_picosec starttime){
    ...
    route_t* routeout = new route_t(*(net_paths[src][dst]->at(choice)));
